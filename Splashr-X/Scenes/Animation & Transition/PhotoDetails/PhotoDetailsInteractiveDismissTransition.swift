//
//  PhotoDetailsInteractiveDismissTransition.swift
//  Splashr-X
//
...
public class PhotoDetailsInteractiveDismissTransition: NSObject {
  ...
  fileprivate weak var toDelegate: PhotoDetailsTransitionAnimatorDelegate?
  ...
  fileprivate weak var fromVC: PhotoDetailsViewController? = nil
  ...
  init(fromDelegate: PhotoDetailsViewController, toDelegate: Any) {
    self.fromDelegate = fromDelegate
    self.toDelegate = toDelegate as? PhotoDetailsTransitionAnimatorDelegate
  ...
  func didPanWith(gestureRecognizer: UIPanGestureRecognizer) {
    ...
    case .ended:
      ...
      let fingerIsMovingUpwards = gestureRecognizer.velocity(in: nil).y < 0
      let transitionMadeSignificantProgress = percentageComplete > 0.1
      let shouldComplete = (fingerIsMovingDownwards && transitionMadeSignificantProgress) || (fingerIsMovingUpwards && transitionMadeSignificantProgress)
  ...
  private func percentageComplete(forVerticalDrag verticalDrag: CGFloat) -> CGFloat {
    ...
    return CGFloat.scaleAndShift(value: abs(verticalDrag), inRange: (min: 0.0, max: maximumDelta))
  }
...
extension PhotoDetailsInteractiveDismissTransition: UIViewControllerAnimatedTransitioning {
...
extension PhotoDetailsInteractiveDismissTransition: UIViewControllerInteractiveTransitioning {
  public func startInteractiveTransition(_ transitionContext: UIViewControllerContextTransitioning) {
    ...
    guard
      ...
      let fromVC = transitionContext.viewController(forKey: .from) as? PhotoDetailsViewController,
    ...
    self.toReferenceImageViewFrame = PhotoDetailsPopTransition.defaultOffscreenFrameForDismissal(
    ...
    toVC.customTabbarController?.setTabBar(hidden: false, animated: true, alongside: animation)
  }
}
